...
| 2026-09-03 | Studio records every coder run (transcript + gate pass/fail) and every reviewer verdict to `data/traces/`; owner corrections via `scripts/override.py` | Free labelled data for fine-tuning; also a debugging record. docs/15-training.md |
| 2026-09-03 | Coder gets retrieval: Godot 4 docs + repo indexed with nomic-embed-text, `search_docs` tool, top chunks in the prompt | Fixes wrong-API and Godot 3 habits without GPU time; done before any training |
| 2026-09-03 | New `train` job kind on the GPU worker with three recipes: SDXL style LoRA (kohya), coder QLoRA+DPO (Unsloth, 7B dense), reviewer QLoRA (Unsloth vision). Datasets built on the server into `assets/training/`, models come back the same way | Fits the pull model and the 12 GB card; no new sync folders. Orchestrator is not fine-tuned (size matters more than tuning there) |
| 2026-09-03 | Nothing activates itself. Finished models are logged; `scripts/activate_model.py` is a human step with rollback; coder switches only after `training/eval_coder.py` beats the baseline | A bad fine-tune must not silently degrade an unattended studio |
| 2026-09-03 | Only gate-passed and owner-approved data trains; failed runs are used only as DPO rejections; public code filtered to Godot 4 projects | Training on your own mistakes entrenches them |
| 2026-09-03 | `AUTO_TRAIN` off by default; thresholds 40 images / 150 passed runs / 100 owner verdicts | Below those counts a fine-tune mostly learns noise |
| 2026-09-03 | Loop law: survival is a decision, never a dice roll. Distance from light is danger; Holdouts and wild Hearths are the prepared player's outs; bases are safe because built; breaches cost things | Owner's day-out/night-home loop, with the coin-flip pushback accepted. docs/17 §1. |
| 2026-09-03 | Decision 3 amended: Keep plot at rung 1, **Holdfasts** (party-claimed ruins, Palworld-style base) at rung 1.5, free-build rung 2 | Resolves shared hub vs personal base. docs/17 §3. |
| 2026-09-03 | Building: tile grid, pieces with cost/health/blocking, roofs need support, power by cable from a relic reactor, light attracts the horde | docs/17 §3, game/data/build.json. |
...
| 2026-09-03 | Weapons: 9 classes with distinct verbs, 25 base weapons, noise as a stat, guns are loud relics; rarities Scrap/Worn/Sound/Fine/Saint; workbench +1..+10; Reliquary socket deterministic and permanent | docs/17 §4. Volume is data, identity is animation. |
| 2026-09-03 | Armor: head/body/legs, light/medium/heavy weight classes, 12 sets with 2/3-piece bonuses and a side effect each, generated from 3 silhouettes per weight | docs/17 §5. |
| 2026-09-03 | Classes: 10 (Reaper, Warden, Gunner, Hunter, Shade, Wright, Linker, Cantor, Physician, Tinker); class = kit + signature relic + talent tree; all gear usable by all; everyone human; nothing is magic | docs/17 §6. Fantasy names reskinned to the world. |
| 2026-09-03 | Workers: robots are repaired relics (Mule, Maintenance drone, Sentry), humans are rescued survivors (Farmer, Smith, Guard, Scout); jobs are stations; 3 workers first | docs/17 §7. Full automation is rung 2. |
