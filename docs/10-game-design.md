...
5. **Data-driven content.** Items, enemies, quests, dialogue, build pieces, wave tables in
   `game/data/*.json`. Code loads them; it never hardcodes them. `weapons.json`, `armor.json`,
   `classes.json` and `rarities.json` are generated by `scripts/gen_items.py` from the stat budget;
...
## Building: the Hold (decision 3, amended 2026-09-03)
...
- Dusk bell two minutes before night (prototype). Holdouts (bar a door) and wild Hearths (burn a
  relic) are how prepared players survive outside.
- A base is safe because you built it. Breaches cost walls, scrap and workers.
- The Long Night every seventh night. Wave pressure scales with light, noise and stored relics.
