...
def dispatch(message, store, project, subproject=''):
    ...
    if method=='initialize':
        ...
        boundary=project+('/'+subproject if subproject else '')
        result={'tools':[{'name':'memory_search','description':'Search '+boundary+' and shared notes; returns status, provenance, source paths and text.',
        ...
        result={'content':[{'type':'text','text':json.dumps(store.search(project,query,subproject=subproject))}]}
    elif method!='ping':
        return {'jsonrpc':'2.0','id':rid,'error':{'code':-32601,'message':'Unknown method'}}
    return {'jsonrpc':'2.0','id':rid,'result':result}

def main():
    project=os.environ.get('ASSISTANT_PROJECT','game')
    subproject=os.environ.get('ASSISTANT_SUBPROJECT','')
    store=Store()
    try:
        for line in sys.stdin:
            if len(line)>1000000:continue
            try:reply=dispatch(json.loads(line),store,project,subproject)
