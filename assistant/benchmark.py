...
def _requalify(root, role, profile, max_age_days, model=None):
...
def switch_model(root, role, model, max_age_days=7):
    """Move one specialist onto one of its approved models.

    Only this role changes. A machine still holds a single model at a time - Ollama runs
    with OLLAMA_MAX_LOADED_MODELS=1, so asking for a different one evicts the resident
    model - but which model that is may differ from job to job, which is the point of
    downloading more than one.

    The cost of that freedom is reload time: two roles on one machine wanting different
    models means several gigabytes are swapped whenever work alternates between them. That
    is a scheduling problem, not a reason to forbid the choice.
    ...
    _requalify(root, role, profile, max_age_days, model)
